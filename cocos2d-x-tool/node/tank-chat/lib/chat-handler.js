...
class Handler {
    constructor() {
        ...
        wee.onExit(async(done) => {
            debug("process onChatExit:", process.pid);
            var queue = [];
            // 删除玩家信息
            for (let [fd, uid] of manage.fdUidList) {
                // 删除玩家信息
                queue.push(this._clearPlayer(uid, fd));
                // 删除Fd会话信息
                queue.push(manage.clearUidFd(uid, fd));
            }
     ...
     * @return {Promise}
     */
    async _clearPlayer(uid) {
        var queue = [];
        var player = await manage.getPlayer(uid);
        if (!player) {
            return false;
        }
        // 退出频道
        if (player.cateId) {
            for (let cateId of player.cateId) {
                queue.push(manage.delCate(cateId, uid));
            }
        }
        // 退出房间
        var tarRoomId = await manage.getPlayerRoom(uid);
        if (tarRoomId) {
            this.remote.emitTo(manage.REMOTE_ROOM_NSP, "offline", player, tarRoomId);
        }
        // 删除玩家信息
        queue.push(manage.delPlayer(uid));
        queue.push(manage.delOnline(uid));
        await Promise.all(queue);
    }
    ...
    async execRoom(fd, cmd, args) {
        ...
        // 转发到房间进程处理
        this.remote.emitTo(manage.REMOTE_ROOM_NSP, "action", player, cmd, args);
    }
    ...
    async worldMsg(fd, data) {
        ...
        var res = await dbModel.useHorn(player.uid);
        if (!res) {
            return manage.send(fd, proto.error("horn_not_enough"));
        }
        var msg = proto.worldMsg(player.uid, player.baseInfo, data);
        await this.remote.emit("sendAll", msg);

        let horn_msg = proto.notice([1,[6,res.res.num]]);
        console.log("send horn_msg " + JSON.stringify(horn_msg));
        this.sendUid(parseInt(player.uid),horn_msg);
    }
