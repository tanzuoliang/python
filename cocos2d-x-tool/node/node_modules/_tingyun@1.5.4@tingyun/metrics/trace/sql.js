...
function SQLTrace(query, action, stats) {
    if (!query) {
        throw new Error("Can't create SQL trace without SQL!");
    }
    if (!action) {
        throw new Error("Can't create SQL trace without action.");
    }
    if (!stats) {
        throw new Error("Can't create SQL trace without statistics.");
    }
    this.query = query;
    this.action = action;
    this.stats = stats;
