...
function createError(action, exception, customParameters, config) {
    ...
    return [
        timestamp,
        (action ? name : 'BackgroundAction/Error/' + exception && exception.name || String(exception)),
        //HTTP_STATUS, dc只识别数字类型，对于System Error.code 传递0
        (action ? action.statusCode : ( /*exception.code || */ 0)),
        type,
        message,
        1,
        action ? action.url : '',
...
ErrorTracer.prototype.onActionFinished = function onActionFinished(action, metrics) {
    if (!action) {
        throw new Error("Error collector got a blank action.");
    }
    if (!metrics) {
        throw new Error("Error collector requires metrics to count errors.");
    }
    var allWebCount;
    var count;
    if (urltils.errorMatch(this.config, action.statusCode) ||
        (action.statusCode < 1 && action.exceptions.length > 0)) {
        var exceptions = action.exceptions.concat(action.userExceptions);
        if (exceptions.length) {
            exceptions.forEach(function cb_forEach(exception) {
                this.add(action, exception);
            }, this);
        } else {
            this.add(action);
        }
        allWebCount = metrics.getMetric('Errors/Count/AllWeb');
        allWebCount.incrementCallCount(1);
        count = metrics.getMetric('Errors/Count/' + action.name.replace(/\//g, "%2F"));
        count.incrementCallCount(1);
    } else if (action && action.userExceptions.length) {
        action.userExceptions.forEach(function(exception) {
            logger.debug('action(%s) notice error', action.name, exception);
            this.add(action, exception);
        }, this);
        allWebCount = metrics.getMetric('Errors/Count/AllWeb');
        allWebCount.incrementCallCount(1);
...
ErrorTracer.prototype.addExternal = function addExternal(segment, metricName, error) {
    ...
    if (this.externalErrors.length < MAX_ERRORS) {
        var timestamp = Math.round((new Date()).getTime() * 0.001);
        var statusCode = res.statusCode;
        var className = 'Error';
        var stacktrace = [];
        if (error) {
            className = error.name || className;
        ...
        logger.verbose("Error to be sent to collector:", {
            error: errorConstruct
        });
        this.externalErrors.push(errorConstruct);
    } else {
        logger.verbose("Already have %d errors to send to collector, not keeping.", MAX_ERRORS);
    }
};
...
ErrorTracer.prototype.add = function add(action, exception, customParameters) {
    if (!exception) {
        ...
        if (this.seen.indexOf(exception) !== -1) {
            return;
        }
