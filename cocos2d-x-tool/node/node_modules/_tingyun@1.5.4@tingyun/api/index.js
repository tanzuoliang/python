...
API.prototype = {
    ...
    noticeError: function(messge, error, isWebAction) {
        if (!arguments.length) {
            logger.warning('illegal argument, missing error info.');
            return this;
        }
        if (messge instanceof Error) {
            error = messge;
            messge = null;
        }
        ...
        if (action) {
            action.forceError = isWebAction;
            action.userExceptions.push(error);
        } else {
    ...
    setWebActionName: function(name) {
        ...
        if (action) {
            ...
            if (action.name) {
                var metricName = 'Errors/Count/' + action.name.replace(/\//g, "%2F");
                var container = this.agent.metrics._container(metricName);
                //reset webaction error count
                if (container && container[metricName]) {
                    container['Errors/Count/' + newName] = container[metricName];
                    delete container[metricName];
                }
            }
            action.name = newName;
        }
