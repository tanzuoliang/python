...
class RoomPve {
    ...
    async info(player, roomId, type, info) {
        ...
        if (manage.TYPE_CUSTOM_MODE.includes(roomInfo.type)) {
            if (type == 1) {
                ...
                await Promise.all([
                    // dbModel.roomChangeInfo(roomId, { typeId: info }),
                    manage.setRoomInfo(roomId, roomInfo)
                ]);
            }
        } else {
    ...
    _roomState(player) {
        return [player.camp, player.pos, player.tankId, player.prop, player.ready];
    }
