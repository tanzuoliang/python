 ...
 * @Last modified time: 18-Oct-2017
 */
"use strict";
// 加载配置文件
...
class Manage {
    ...
    send(fd, msg) {
        ...
        if (!ws) {
            return wee.error("send to FD = ", fd, "not exists");
        }
        if (!wee.isString(msg)) {
    ...
    async clearRoomInfo() {
        var roomIds = await this.redis.keys("room-info:*");
        if (!wee.empty(roomIds)) {
            await this.redis.del(roomIds);
        }
    }
    ...
    async clearRoomTeam() {
        var roomTeamIds = await this.redis.keys("room-team:*");
        if (!wee.empty(roomTeamIds)) {
            await this.redis.del(roomTeamIds);
        }
    }
