...
def hortPack2(projectroot,dirlist,staticfilelist,f,t,store):
	...
	for fi in staticfilelist:
		...
		print "to = ",os.path.join(savePath, base_name)
...
fv = raw_input("input the version where you want to collect from:\n")
tv = raw_input("input the version where you want to collect to:\n")
st_f = []
...
if "game_version.json" in game_version:
	...
	with open(game_version.strip(),"r") as f:
		j = json.load(f)
		curcode = j["version"]
#	base_name = os.path.dirname(game_version)
#	if "/" in base_name:
#		ls = base_name.split("/")
#		if len(ls) >= 1:
#			versionType = ls[-1]	
...
hortPack2(os.getcwd(),['res','src'],st_f,fv,tv,"hot_out/hot_res_%s_r%s-%s"%(curcode,fv,tv))